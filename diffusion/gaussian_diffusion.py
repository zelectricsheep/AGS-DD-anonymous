...
class GaussianDiffusion:
    ...
    def p_sample_with_mode_guidance(
        ...
        xstart, _ = out["pred_xstart"].chunk(2, dim=0)
        if t[0].item() > stop_t:
            guidance_score = - (xstart - mode_features) * mode_guidance_scale * th.exp(0.5 * out["log_variance"])
        else:
            guidance_score = th.tensor([0], device=xstart.device)

        # print(t[0].item(), th.max(noise), th.max(th.exp(0.5 * out["log_variance"])), guidance_score.max())
        # print(t)
        w = 0.9
        sample = out["mean"] + guidance_score + nonzero_mask * th.exp(0.5 * out["log_variance"]) * noise
        return {"sample": sample, "pred_xstart": out["pred_xstart"]}
